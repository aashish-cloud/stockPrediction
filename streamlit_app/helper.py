...
def combined_stock_prediction(stock_ticker):
    ...
    try:
        ...
        train_date_col = date_col[:int(training_data_len)]
        ...
        print("Mean Squared Error on training:", mse)
        ...
        test_date_col = date_col[int(training_data_len):]

        predictions = model.predict(x_test)

        min_e = np.min(dataset[:,0])
        max_e = np.max(dataset[:,0])

        predictions =  predictions * (max_e - min_e) + min_e
        y_test = y_test * (max_e - min_e) + min_e 

        print(y_test[:10], predictions[:10])
        print("mean squared error on testing: ", mean_squared_error(y_test, predictions))
        ...
        train_df = pd.DataFrame(x_train, columns=selected_columns)
        train_df.set_index(pd.Index(train_date_col))

        # train_df = pd.merge(train_date_col, train_d, left_index=True, right_index=True, how='outer')


        x_test = scaler.inverse_transform(x_test)
        test_df = pd.DataFrame(x_test, columns=selected_columns)
        test_df.set_index(pd.Index(test_date_col))
        # test_df = pd.merge(test_date_col, test_d, left_index=True, right_index=True, how='outer')
